...
class Inscription {
	...
	public static function activer($pseudo) {
		$activation = Bdd::connectBdd()->prepare(SELECT.ALL.ACTIVATION.METHODEACTIV);//WHERE activation=1
		$activation -> execute();
...
class Connexion {
	...
	public static function jeton($login) {
		...
		$ip = Ip::get_ip();
		$date = time();
		$resultat = Bdd::connectBdd()->prepare(SELECT.ALL.JETON.JETONCONNEXION);// select *FROM JejeScriptSecure  WHERE id_membre=:id AND ip_connexion=:ip
		$resultat -> bindParam(':id', $id, PDO::PARAM_INT, 11);
		...
		if($resultat -> rowCount() === 1) {
			$donnee = $resultat->fetch(PDO::FETCH_ASSOC);
			$id = Membre::recupId($login);
			$maj = Bdd::connectBdd()->prepare(UPDATE.JETONZ.JETONDATE.JETONMEMBRE);//UPDATE JejeScriptSecure  SET date=:date  WHERE id_membre=:id
			$maj -> bindParam(':id', $id);
			...
			$insert = Bdd::connectBdd()->prepare(INSERT.JETONZ.JETONVALUES);//INSERT JejeScriptSecure (id_membre, jeton, ip_connexion, date) VALUES (:id, :jeton, :ip, :date)
...
class Activation {
	...
	public static function activationEnvoiMail($login, $jeton) {
		...
		$message .= 'Cordialement,'."\n";
		$message .= NOMSITE.'.'."\n";
		mail(MAILSITE, $sujet, $message, $headers);
	}
...
class ProtectEspace {
	...
	public static function administrateur($id, $captcha, $jeton, $niveau) {
		if(empty($id) OR empty($captcha) OR empty($jeton)) {
			...
			$resultat = Bdd::connectBdd()->prepare(SELECT.ALL.JETON.JETONSESSION); //SELECT * FROM JejeScriptSecure WHERE id_membre=:id AND jeton=:jeton
			$resultat -> bindParam(':id', $id, PDO::PARAM_INT, 11);
			$resultat -> bindParam(':jeton', $jeton);
			$resultat -> execute();
			if($resultat -> rowCount() !== 1) {
	...
	public static function compteJeton($id) {
		$resultat = Bdd::connectBdd()->prepare(SELECT.ALL.JETON.JETONMEMBRE);//SELECT*FROM JejeScriptSecure
		$resultat -> bindParam(':id', $id, PDO::PARAM_INT, 11);
		$resultat -> execute();
	...
	public static function listeJeton($id) {
		$liste = '';
		$resultat = Bdd::connectBdd()->prepare(SELECT.ALL.JETON.JETONMEMBRE);//SELECT * FROM JejeScriptSecure WHERE id_membre=:id
		$resultat -> bindParam(':id', $id, PDO::PARAM_INT, 11);
		$resultat -> execute();
	...
	public static function deleteJeton($id) {
		$resultat = Bdd::connectBdd()->prepare(DELETE.JETON.ID);//DELETE FROM JejeScriptSecure WHERE id=:id
		$resultat -> bindParam(':id', $id, PDO::PARAM_INT, 11);
		$resultat -> execute();
	}
...
class Membre {
	...
	public static function profilVisibilite($id, $info) {
		$resultat = Bdd::connectBdd()->prepare(SELECT.ALL.ACCESPROFIL.IDMEMBRE);//JejeScriptAccesFiches
		$resultat -> bindParam(':id', $id, PDO::PARAM_INT, 11);
		$resultat -> execute();
		$donnee = $resultat -> fetch(PDO::FETCH_ASSOC);
		if($donnee[$info] === '1') {
...
class InfoSite {
	...
	public static function activationChange($id) {
		$activ = '1';
		$oui = Bdd::connectBdd()->prepare(UPDATE.ACTIVATIONZ.CHANGEMETOD.ID);//UPDATE JejeScriptActivation SET activation=:activ WHERE id=:id
		$oui -> bindParam(':activ', $activ);
		$oui -> bindParam(':id', $id, PDO::PARAM_INT, 11);
		if($oui -> execute()) {
			$desac = '0';
			$non = Bdd::connectBdd()->prepare(UPDATE.ACTIVATIONZ.CHANGEMETOD.NOI);//UPDATE JejeScriptActivation SET activation=:activ WHERE id!=:id
...
class Admin {
	...
	public static function bannir($id, $messagePost) {
		$activ = '5';
		$resultat = Bdd::connectBdd()->prepare(UPDATE.MEMBREZ.ACTIVMEMBRE.ID);//JejeScriptMembres  SET activation=:activer
		$resultat -> bindParam(':activer', $activ);
		$resultat -> bindParam(':id', $id, PDO::PARAM_INT, 11);
		$resultat -> execute();
		//				*******************				//
