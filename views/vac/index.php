...
"submitvalue" => 'Search',
"cb1"         => $ctrl,        "mb1" => 'vac/'.$this->userListviewq,         "tb1" => 'Créer un nouveau acte de vaccination ',      "vb1" => '',  "icon1" => 'add.PNG',
...
<?php
		...
		echo '<br>';
		echo'<table>';
			echo'<tr bgcolor="#00CED1"><th colspan="'.$colspan.'" ><A HREF="'.URL.$ctrl.'/'.$mdl.'/'.$urlx[2].'/'.$urlx[3].'?o=IDELEVE&q=6"> La liste des actes de vaccination </A> : <span>'.$total_count1.'/'.$total_count.' enregistrement(s) trouvé(s)</span></th></tr>';
			echo'<tr bgcolor="#00CED1">';
			echo'<th >DCI vaccin</th>';
			echo'<th >Date vaccin</th>';
			echo'<th >Date péremption</th>';
			echo'<th >N° LOT</th>';
	        echo'<th >Palier</th>';
			...
			echo '<td align="center"  >'.HTML::nbrtostring("vaccin","id",$value['vaccin'],"vaccin").'</td>';
			echo '<td align="center"  >'.HTML::dateUS2FR($value['datevac']).'</td>';
			...
			if ($total_count <= 0 ){header('location: ' . URL .$ctrl.'/vac/'.$this->userListviewq);	}else{echo '<tr bgcolor="#00CED1"><td id="bdn"  colspan="'.$colspan.'" >'. HTML::barre_navigation ($total_count,$this->userListviewl,$this->userListviewo,$this->userListviewq,$this->userListviewp,$this->userListviewb,$ctrl,$mdl,$order).'</td></tr>';		 	}	
		echo "</table>";
		echo "</div>";
		ob_end_flush();
		}
