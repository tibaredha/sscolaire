...
<div class="sheader2l">
Hygiene et salubrité des locaux UDS : 
<?php
$ctrl='hsl';$mdl='search';
$data = array(
"c"           => $ctrl,
...
"submitvalue" => 'Search',
"cb1"         => $ctrl,        "mb1" => 'hsl/'.$this->userListviewq,         "tb1" => 'Créer un nouveau Hygiene et salubrité des locaux ',      "vb1" => '',  "icon1" => 'add.PNG',
"cb2"         => "dashboard",  "mb2" => 'search/0/10?o=id&q=',               "tb2" => 'Eleves scolarisés',                                  "vb2" => '',  "icon2" => 'eva.png');
echo  HTML::nbrtostring('uds','id',Session::get('uds'),'uds');
?>
...
function anomalie ($xx,$yy,$zz){if($xx==1) {return  $yy;}else{return  $zz;} }
		...
		echo '<br>';
		echo'<table>';
			echo'<tr bgcolor="#00CED1"><th colspan="'.$colspan.'" ><A HREF="'.URL.$ctrl.'/'.$mdl.'/'.$urlx[2].'/'.$urlx[3].'?o=IDELEVE&q='.$this->userListviewq.'"> La liste des Hygienes et salubrités des locaux </A> : <span>'.$total_count1.'/'.$total_count.' enregistrement(s) trouvé(s)</span></th></tr>';
			...
			if ($total_count <= 0 ){header('location: ' . URL .$ctrl.'/hsl/'.$this->userListviewq);	}else{echo '<tr bgcolor="#00CED1"><td id="bdn"  colspan="'.$colspan.'" >'. HTML::barre_navigation ($total_count,$this->userListviewl,$this->userListviewo,$this->userListviewq,$this->userListviewp,$this->userListviewb,$ctrl,$mdl,$order).'</td></tr>';		 	}	
		echo "</table>";
		echo "</div>";
		ob_end_flush();
		}
