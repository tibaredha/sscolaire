...
class bnm_Model extends Model {
    ...
    return $this->db->select("SELECT * FROM bordereau where  $o like '$q%' order by $o $ad  limit $p,$l ");// 
    }
    public function userSearch1($o, $q,$ad) {
        $structure = Session::get("structure");
		return $this->db->select("SELECT * FROM bordereau where $o like '$q%' order by $o $ad");//  
    }
