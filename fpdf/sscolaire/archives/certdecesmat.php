...
{	
	...
	$pdf->SetXY(64,36);$pdf->Write(0,'ETABLISSEMENT DE SANTE : '.$pdf->nbrtostring('structure','id',intval(trim($row->STRUCTURED)),'structure'));
	$pdf->SetFont('Arial','B',10);
	...
	$pdf->SetXY(73,46+28);$pdf->Write(0,$row->ADRESSE.'_'.$pdf->nbrtostring('com','IDCOM',$row->COMMUNER,'COMMUNE'));
    //LIEU DU DECES
	switch($row->LD)  
		{
			case 'DOM':
	...
	$pdf->SetXY(60,111.5+24+6);$pdf->Write(0,$pdf->nbrtostring('servicedeces','id',intval(trim($row->SERVICEHOSPIT)),'service'));	
...
// $pdf->SetXY(146,208+23);$pdf->Write(0,"X");	
	// }	
$pdf->SetXY(28,244.5); $pdf->Write(0,$pdf->dateUS2FR($row->DINS));
$pdf->SetXY(100,244.5);$pdf->Write(0,$pdf->nbrtostring('com','IDCOM',$row->COMMUNED,'COMMUNE'));
$pdf->SetXY(115,244.5+12);$pdf->Write(0,$row->MEDECINHOSPIT);
